/*
  File: Roller5.java
  This version based on Lagrangian method for finding equation of motion.
...
public class Roller5 extends Simulation implements ObjectListener {
  ...
  public double getVariable(int i) {
    if (i<=2)
      return vars[i];
    if (i==3) {   // velocity in terms of track length parameter
      // return Sqrt(1+(-(7/3)x + (2/3)x^3)^2) * dx/dt
      double x = vars[0];
...
/*
  ...
  */
  ...
  x' = v
      ...
      -(x*(-7 + 2*Power(x,2))*(3*g + (-7 + 6*Power(x,2))* Power(v,2)))
 v' = ----------------------------------------------------
      (9 + 49*Power(x,2) - 28*Power(x,4) + 4*Power(x,6))
  */
  public void evaluate(double[] x, double[] change) {
    double x2 = x[0]*x[0];
    change[0] = x[1];  // x' = v
